...
const (
	FeatureGlobal       FeatureFlags = 1 << iota /* Feature applies to all characters; results in no mask allocated for it. */
	FeatureHasFallback                           /* Has fallback implementation, so include mask bit even if feature not found. */
	FeatureManualZWNJ                            /* Don't skip over ZWNJ when matching **context**. */
	FeatureManualZWJ                             /* Don't skip over ZWJ when matching **input**. */
	FeatureGlobalSearch                          /* If feature not found in LangSys, look for it in global feature list and pick one. */
	FeatureRandom                                /* Randomly select a glyph from an AlternateSubstFormat1 subtable. */
	FeaturePerSyllable                           /* Contain lookup application to within syllable. */
)

const (
	FeatureNone                FeatureFlags = 0
	FeatureManualJoiners                    = FeatureManualZWNJ | FeatureManualZWJ
	FeatureGlobalManualJoiners              = FeatureGlobal | FeatureManualJoiners
	FeatureGlobalHasFallback                = FeatureGlobal | FeatureHasFallback
)

const (
	ffGLOBAL              = FeatureGlobal
	ffHasFallback         = FeatureHasFallback
	ffManualZWNJ          = FeatureManualZWNJ
	ffManualZWJ           = FeatureManualZWJ
	ffGlobalSearch        = FeatureGlobalSearch
	ffRandom              = FeatureRandom
	ffPerSyllable         = FeaturePerSyllable
	ffNone                = FeatureNone
	ffManualJoiners       = FeatureManualJoiners
	ffGlobalManualJoiners = FeatureGlobalManualJoiners
	ffGlobalHasFallback   = FeatureGlobalHasFallback
)
...
func (mb *otMapBuilder) AddFeatureExt(tag tables.Tag, flags FeatureFlags, value uint32) {
...
type gsubPauseContext struct {
	font   *Font
	buffer *Buffer
}

func (ctx gsubPauseContext) Font() *Font     { return ctx.font }
func (ctx gsubPauseContext) Buffer() *Buffer { return ctx.buffer }

func (mb *otMapBuilder) addPause(tableIndex int, fn GSUBPauseFunc) {
	s := stageInfo{
...
func (mb *otMapBuilder) HasFeature(tag tables.Tag) bool {
...
func (m *otMap) position(_ *otShapePlan, font *Font, buffer *Buffer) {
	if debugMode {
...
func (m *otMap) apply(proxy otProxy, font *Font, buffer *Buffer) {
	...
	for stageI, stage := range m.stages[tableIndex] {
		...
		if stage.pauseFunc != nil {
			...
			stage.pauseFunc(gsubPauseContext{font: font, buffer: buffer})
		}
	}
}
