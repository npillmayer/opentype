...
func hebrewCompose(c harfbuzz.NormalizeContext, a, b rune) (rune, bool) {
...
func hebrewReorderMarks(buffer *harfbuzz.Buffer, start, end int) {
	info := buffer.Info

	for i := start + 2; i < end; i++ {
		c0 := info[i-2].ModifiedCombiningClass()
		c1 := info[i-1].ModifiedCombiningClass()
		c2 := info[i].ModifiedCombiningClass()
