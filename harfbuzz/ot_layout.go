...
func (c *otApplyContext) applyForward(accel *otLayoutLookupAccelerator) bool {
	...
	for buffer.idx < len(buffer.Info) {
		applied := false
		if (buffer.cur(0).Mask&c.lookupMask) != 0 &&
...
func clearSubstitutionFlags(ctx PauseContext) bool {
	buffer := ctx.Buffer()
	info := buffer.Info
	for i := range info {
...
func reverseGraphemes(b *Buffer) {
	b.reverseGroups(func(_, gi2 *GlyphInfo) bool { return gi2.isContinuation() }, true)
}
