...
// AddRune appends a character with the Unicode value of `codepoint` to `b`.
// The cluster value is assigned automatically as the next input index.
// This also clears the posterior context (see `AddRunes`).
func (b *Buffer) AddRune(codepoint rune) {
	b.append(codepoint, len(b.Info))
	b.clearContext(1)
}
...
// MergeClusters merges cluster IDs in [start:end] and extends to cover
// neighboring glyphs that already share the same cluster values.
func (b *Buffer) MergeClusters(start, end int) {
...
// UnsafeToBreak marks the range as unsafe-to-break (and unsafe-to-concat).
func (b *Buffer) UnsafeToBreak(start, end int) {
...
// SafeToInsertTatweel marks the range as safe insertion points for tatweel.
func (b *Buffer) SafeToInsertTatweel(start, end int) {
	b.safeToInsertTatweel(start, end)
}
...
// UnsafeToConcat marks the range as unsafe-to-concat.
func (b *Buffer) UnsafeToConcat(start, end int) {
	b.unsafeToConcat(start, end)
}
...
// UnsafeToConcatFromOutbuffer marks out-buffer/frontier clusters as
// unsafe-to-concat.
func (b *Buffer) UnsafeToConcatFromOutbuffer(start, end int) {
	b.unsafeToConcatFromOutbuffer(start, end)
}

// PreContext returns a copy of the text context before the main buffer.
func (b *Buffer) PreContext() []rune {
	return append([]rune(nil), b.context[0]...)
}

// PostContext returns a copy of the text context after the main buffer.
func (b *Buffer) PostContext() []rune {
	return append([]rune(nil), b.context[1]...)
}
...
func (b *Buffer) infosSetGlyphFlags(infos []GlyphInfo, start, end, cluster int, mask GlyphMask) {
	...
	if cluster != clusterFirst && cluster != clusterLast {
		for i := start; i < end; i++ {
