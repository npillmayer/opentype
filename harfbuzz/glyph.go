...
type GlyphInfo struct {
	...
	// Cluster merging follows monotone grapheme semantics.
	Cluster int
...
// ModifiedCombiningClass returns the shaping modified combining class.
func (info GlyphInfo) ModifiedCombiningClass() uint8 {
	return info.getModifiedCombiningClass()
}
...
// SetModifiedCombiningClass sets the shaping modified combining class.
func (info *GlyphInfo) SetModifiedCombiningClass(modifiedClass uint8) {
	info.setModifiedCombiningClass(modifiedClass)
}

// Codepoint returns the glyph input Unicode scalar value.
func (info GlyphInfo) Codepoint() rune { return info.codepoint }

// SetCodepoint updates the glyph input Unicode scalar value.
func (info *GlyphInfo) SetCodepoint(r rune) { info.codepoint = r }

// ComplexAux returns the shaper-specific auxiliary byte.
func (info GlyphInfo) ComplexAux() uint8 { return info.complexAux }

// SetComplexAux updates the shaper-specific auxiliary byte.
func (info *GlyphInfo) SetComplexAux(v uint8) { info.complexAux = v }

// GeneralCategory returns the Unicode general category as a numeric value.
func (info GlyphInfo) GeneralCategory() uint8 { return uint8(info.unicode.generalCategory()) }
...
// LigComp returns ligature component metadata used by GSUB/GPOS shaping.
func (info GlyphInfo) LigComp() uint8 { return info.getLigComp() }
...
// IsDefaultIgnorable reports whether this glyph is default-ignorable.
func (info GlyphInfo) IsDefaultIgnorable() bool { return info.isDefaultIgnorable() }
...
// Multiplied reports whether this glyph came from multiple substitution.
func (info GlyphInfo) Multiplied() bool { return info.multiplied() }
