...
// HebrewCompose handles Hebrew-specific composition overrides.
func HebrewCompose(c NormalizeContext, a, b rune) (rune, bool) {
	ab, found := c.ComposeUnicode(a, b)

	if !found && !c.HasGposMark() {
...
// HebrewGposTag is the preferred GPOS script tag for Hebrew shaping.
func HebrewGposTag() ot.Tag {
	// https://github.com/harfbuzz/harfbuzz/issues/347#issuecomment-267838368
	return ot.NewTag('h', 'e', 'b', 'r')
}

// HebrewReorderMarks applies Hebrew-specific mark reordering.
func HebrewReorderMarks(buffer *Buffer, start, end int) {
	info := buffer.Info
