...
const (
	...
	NormalizationComposedDiacritics               // never composes base-to-base
	NormalizationComposedDiacriticsNoShortCircuit // always fully decomposes and then recompose back

	NormalizationAuto    // see below for logic.
	NormalizationDefault = NormalizationAuto
)

const (
	nmNone                             = NormalizationNone
	nmDecomposed                       = NormalizationDecomposed
	nmComposedDiacritics               = NormalizationComposedDiacritics
	nmComposedDiacriticsNoShortCircuit = NormalizationComposedDiacriticsNoShortCircuit
	nmAuto                             = NormalizationAuto
	nmDefault                          = NormalizationDefault
)
...
type otNormalizeContext struct {
	...
	buffer *Buffer
	font   *Font
	norm   otNormalizeBackend
	// hb_unicode_funcs_t *unicode;
	decompose func(c NormalizeContext, ab rune) (a, b rune, ok bool)
	compose   func(c NormalizeContext, a, b rune) (ab rune, ok bool)
}

var _ NormalizeContext = (*otNormalizeContext)(nil)

func (c *otNormalizeContext) modifiedCombiningClass(u rune) uint8 {
	// Keep HarfBuzz-specific modified-class behavior, but source canonical CCC
	// from the active normalization backend.
	switch u {
	case 0x1A60:
		return 254
	case 0x0FC6:
	...
	case 0x0F39:
		return 127
	default:
		return modifiedCombiningClass[c.canonicalCombiningClass(u)]
	}
}

func (c *otNormalizeContext) refreshGlyphCombiningClass(info *GlyphInfo) {
	info.setModifiedCombiningClass(c.modifiedCombiningClass(info.codepoint))
}

func (c *otNormalizeContext) DecomposeUnicode(ab rune) (a, b rune, ok bool) {
	return c.norm.decompose(ab)
}

func (c *otNormalizeContext) ComposeUnicode(a, b rune) (ab rune, ok bool) {
	return c.norm.compose(a, b)
}

func (c *otNormalizeContext) HasGposMark() bool {
	return c.plan.hasGposMark
}

func (c *otNormalizeContext) canonicalCombiningClass(u rune) uint8 {
	return c.norm.canonicalCombiningClass(u)
}
...
func outputChar(c *otNormalizeContext, buffer *Buffer, unichar rune, glyph GID) {
	buffer.cur(0).Glyph = glyph
	buffer.outputRune(unichar) // this is very confusing indeed.
	buffer.prev().setUnicodeProps(buffer)
	c.refreshGlyphCombiningClass(buffer.prev())
}

func nextChar(c *otNormalizeContext, buffer *Buffer, glyph GID) {
	buffer.cur(0).Glyph = glyph
	c.refreshGlyphCombiningClass(buffer.cur(0))
	buffer.nextGlyph()
}
...
func decompose(c *otNormalizeContext, shortest bool, ab rune) int {
	...
	if shortest && hasA {
		/// output a and b
		outputChar(c, buffer, a, aGlyph)
		if b != 0 {
			outputChar(c, buffer, b, bGlyph)
			return 2
		}
		return 1
	}
