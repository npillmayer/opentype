...
// In practice, it seems useless to do shaping without a valid face,
// so we skip these test cases.
func skipUnsupportedFont(t *testing.T, ft font.FontID) bool {
	...
	face, err := font.NewFont(fonts[ft.Index])
	tu.AssertNoErr(t, err)

	// OT-only build: skip tests that require AAT tables.
	if len(face.Morx) != 0 || face.Kerx != nil || !face.Trak.IsEmpty() {
		t.Logf("skipping AAT-dependent font %s", ft.File)
		return true
	}
	// GPOS-only kerning policy: skip fonts that only expose legacy 'kern' data.
	if face.Kern != nil && len(face.GPOS.Lookups) == 0 {
		t.Logf("skipping legacy-kern-only font %s", ft.File)
		return true
	}
...
func isSupportedScript(script language.Script) bool {
	switch script {
	case 0,
		language.Unknown, language.Common, language.Inherited,
		language.Latin, language.Cyrillic,
		language.Hebrew, language.Arabic, language.Syriac:
		return true
	default:
		return false
	}
...
func skipUnsupportedScript(t *testing.T, test testData) bool {
	script := test.input.shaper.props.Script
	if script == 0 {
		for _, r := range test.input.text {
			s := language.LookupScript(r)
			if s.Strong() && s != language.Unknown {
				script = s
				break
			}
		}
	...
	if !isSupportedScript(script) {
		t.Logf("skipping unsupported script %v", script)
		return true
	}
...
func skipOutOfScopeLanguage(t *testing.T, test testData) bool {
	lang := strings.ToLower(string(test.input.shaper.props.Language))
	if strings.HasPrefix(lang, "zh") || strings.HasPrefix(lang, "yue") || strings.HasPrefix(lang, "cmn") {
		t.Logf("skipping out-of-scope language %q", lang)
		return true
	}
	return false
}
...
func runShapingTest(t *testing.T, test testData, skipVerify bool) string {
	...
	if skipUnsupportedFont(t, test.input.fontOpts.fontRef) {
		return ""
	}
	if skipUnsupportedScript(t, test) {
		return ""
	}
	if skipOutOfScopeLanguage(t, test) {
		return ""
	}
