...
// ---We manage a navigation path as a stack-----------------------------
//
// pathNode is a node in the navigation path.
type pathNode struct {
	table ot.Table
	key   string
	inx   int
	kind  nodeKind // ---selector for tagged “union”---
	nav   ot.Navigator
	m     ot.NavMap
	tm    ot.TagRecordMap
	list  ot.NavList
...
func (n *pathNode) hasLocation() bool {
	switch n.kind {
	case nodeNav:
		return n.nav != nil
	case nodeMap:
...
// Commands for the interpreter will consist of a sequence of op-codes.
// Each op may have an optional string argument.
type Op struct {
	code   int
	arg    string
	format string
...
// A command is a sequence of op-codes.
type Command struct {
	count int
	op    [32]Op
}
...
func (intp *Intp) parseCommand(line string) (*Command, error) {
	...
	for i, step := range steps {
		...
		//
		if arg, ok := command.op[i].hasArg(); ok {
			//command.op[i].arg = strings.ToLower(command.op[i].arg)
			tracer().Infof("op: %s: looking for '%s'", opNames[command.op[i].code], arg)
		} else {
			tracer().Infof("op: %s", opNames[command.op[i].code])
		}
	}
	return &command, nil
}
...
// quitOp prints a goodbye message and exits the program.
func quitOp(intp *Intp, op *Op) (error, bool) {
	pterm.Println("Goodbye!")
	return nil, true
}

// navigateOp follows a navigation link and puts the target onto the stack.
func navigateOp(intp *Intp, op *Op) (error, bool) {
	if intp.table == nil {
		...
		n := pathNode{kind: nodeNav, nav: l.Navigate(), inx: -1}
		intp.stack = append(intp.stack, n)
		tracer().Infof("walked to %s", n.nav.Name())
	}
	return nil, false
}

// popOp removes the top element from the stack, if present.
...
func (intp *Intp) checkNode() (n pathNode, err error) {
	...
	if n.kind == nodeNav && n.nav.IsVoid() {
		err = ERR_VOID
	}
	return
}
...
// String returns a string representation of the interpreter navigation stack.
...
func (n *pathNode) locationName() string {
	switch n.kind {
	case nodeNav:
		if n.nav != nil {
			return n.nav.Name()
